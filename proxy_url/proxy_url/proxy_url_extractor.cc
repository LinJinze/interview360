...
namespace qh
{
	...
	//get arguments
		std::string url_data = raw_url.substr(raw_url.find("?", 0) + 1, raw_url.length());
        std::string key;
		std::string onedata;
		int and_position = 0;
		int equ_position = 0;
		while (url_data.length() > 0) {
			and_position = url_data.find("&", 0);
			//get one argument
			onedata = url_data.substr(0, and_position);
			//cut arguments
			if(and_position == -1)
			{
				url_data = "";
			...
			{
				url_data = url_data.substr(and_position + 1, url_data.length());
			}
			equ_position = onedata.find("=",0);
			//get key
			if(equ_position == -1)
			{
				key = "";
			...
			{
				key = onedata.substr(0, equ_position);
			}
			//set value
            if (keys.find(key) != keys.end()) 
			{
				sub_url = onedata.substr(equ_position + 1, onedata.length());
            }
        }
#else
